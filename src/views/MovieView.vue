...
export default {
	...
	methods: {
		async fetchMovie(id) {
 			const movieResponse= await axios.get(
				'https://api.themoviedb.org/3/movie/'+id+'?api_key=f4a943efca00a3cd96ac56ff8ad1ea3c'
			);
    ...
    async fetchActors(id) {
 			const actorResponse= await axios.get(
				'https://api.themoviedb.org/3/movie/'+id+'/credits?api_key=f4a943efca00a3cd96ac56ff8ad1ea3c'
			);
...
<template>
  <div class="title">
    ...
    <!-- {{ movie.release_date.split("-")[0] }} --> <!--but this somehow doesn't-->
  </div>
